...
{
  int l_s = ft_strlen(s_sbr);
  int has_sign = (modifier->plus || modifier->space || nbr < 0);
  int count = max(max(modifier->with, modifier->precision) , ft_strlen(s_sbr));
...
int	ft_put_l(t_modifier *modifier, long nbr)
{
  ...
  int count = l_count(modifier, nbr, s);
	...
	if (modifier->minus)
	  	l_handle_with(modifier, s, nbr);
  free(s);
	//return (max(modifier->with , l_s) + (modifier->plus || modifier->space || nbr < 0));
  return count;
}
